...
- 右侧任务上下文：默认顺序固定为任务拆解、任务产出物、上下文与资源。任务拆解不能写死固定五步，必须优先且基本只消费 Hermes 显式 `todo/steps/task.plan` 类结构化计划；Hermes 未暴露正式拆解时，只显示“等待 Hermes 规划 / 任务已完成 / 任务失败”等轻量状态，不能再从 thinking/status/tool/artifact/complete 事件推导假步骤。工具调用、网页、文件、Skill 归入“上下文与资源”或过程记录，不污染任务拆解。步骤命名必须保留 Hermes todo 的主题信息，只做长度压缩；Plan、ReAct、Reflection、Result 只作为每步后面的中文小标签（计划/行动/校验/结果），不能在顶部铺成静态模式条；表情化 thinking、后台心跳、`The user is`、`reasoning.available`、`Hermes 已返回最终结果` 等原始事件不能作为用户可见步骤或说明。
...
- 右侧工作区已按用户掌控感重新收敛为三块默认信息：任务拆解、任务产出物、上下文与资源；任务拆解以 Hermes `todo` 结构化计划为唯一主数据源，最多展示 3-6 个与用户目标直接相关的步骤。工具/网站/文件放入上下文与资源或过程记录，Skill 作为常驻资源保留，Hermes Session、运行时、阈值、原始日志等退入后台诊断或按需动作，不作为默认静态信息铺开。
...
这个测试验证右侧任务拆解只使用 Hermes `todo` 计划：有 `todos` 时保留 todo 的主题标题和状态；只有搜索、文件、工具调用事件时，不生成假步骤。
...
12. 工程稳定阶段可以收尾：`App.tsx` 当前主要职责是页面组合、全局入口互联和少量 modal 开关；继续拆分收益已经低于进入文件预览与编辑阶段。下一阶段新增文件编辑、主题化、客户端化能力时，必须先落到对应 feature 模块，再让 `App.tsx` 只做组合。
13. 每一刀都必须先跑 `npm run -s typecheck`，涉及前端渲染的再跑 `npm run -s build` 和浏览器验证。
