...
6. 样式主题化第一刀完成：已拆 `apps/web/src/styles/tokens.css`、`base.css`、`shell.css`、`sidebar.css`、`chat.css`；`app.css` 继续作为业务组件样式入口。下一步可拆 `settings.css`、`workspace.css` 和 `file-preview.css`。
