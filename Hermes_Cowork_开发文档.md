...
- 后续新增 `diff_card`、`tool_card`、`activity_group` 时，都应优先扩展这里的 part 类型和 renderer，不要在 `App.tsx` 或多个组件里分散判断。
...
10. 对话区 `MessagePart` 第一刀完成：已新增 `apps/web/src/features/chat/MessageParts.tsx`，把正文、附件、文件引用、产物和人工审批统一成 message parts；`MessageBody.tsx` 退回轻量 wrapper；新增 `apps/web/test/message_parts.test.ts` 验证“文件清单表格转文件卡片、真实数据表格保留 Markdown 表格、未解决审批生成 approval_card”。
