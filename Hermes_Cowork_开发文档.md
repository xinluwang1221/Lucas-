...
- 后续新增 `diff_card` 时，应优先扩展这里的 part 类型和 renderer，不要在 `App.tsx` 或多个组件里分散判断。
...
10. 对话区 `MessagePart` 第一刀完成：已新增 `apps/web/src/features/chat/MessageParts.tsx`，把正文、附件、文件引用、产物、人工审批、运行中工具面板和完成后过程摘要统一成 message parts；`MessageBody.tsx` 退回轻量 wrapper，`ChatExecutionViews.tsx` 不再自行构造运行过程 UI；新增 `apps/web/test/message_parts.test.ts` 验证“文件清单表格转文件卡片、真实数据表格保留 Markdown 表格、未解决审批生成 approval_card、过程摘要生成 activity_group、运行中工具生成 tool_card”。
