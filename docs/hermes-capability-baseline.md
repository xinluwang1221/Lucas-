...
| MCP | stdio / HTTP server、自动发现、工具前缀、server filtering、Hermes 自身作为 MCP server | 已覆盖服务、测试、启停、工具 include/exclude、OAuth 重新授权、Hermes MCP serve；configure 交互式命令和远程 MCP 权限仍需深化 | MCP 页要表达“外部能力管理”，而不是只展示配置。 |
...
- MCP configure 原生交互式命令、远程 HTTP MCP 权限说明、Hermes 作为 MCP server 的生产级管理。
